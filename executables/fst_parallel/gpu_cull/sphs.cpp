...
std::string title = "First Parallel Version GPU culling"; 
...
int main(int argc, char* argv[]) 
{
    ...
    ComputeShader computeShader("assets/shaders/fst_parallel_attempt/gpu_cull/sphere_culling.compute");
    ...
    frustumSpheres = actualSphereCount;
    ...
    Profiler profiler(window, "media/off/fst_parallel/gpu_frame_times.off", "media/off/fst_parallel/gpu_process_times.off", actualSphereCount, 0);
    ...
    try
    {
        ...
        while ( isRunning )
        {
            ...
            computeShader.use();
            computeShader.setInt("sphereCount", actualSphereCount);
            setFrustumUniforms(computeShader, frustum);
