...
__global__ void cleaningKernelNoOcc(
    ...
    )
{
    ...
    if (x >= cst.c_screenResolutionX || y >= cst.c_screenResolutionY) return;

    depthBuffer[x + cst.c_screenResolutionX * y] = __float_as_uint(cst.c_far);

    uchar4 color = make_uchar4(backgroundColor.x * 255, backgroundColor.y * 255, backgroundColor.z * 255, 255); 

    // Escritura directa a la textura 2D
    surf2Dwrite(color, surface, x * sizeof(uchar4), y);
}

extern "C" void cleaningScreenNoOcc(
    ...
    nvtxRangePushA("Cleaning No Occ Kernel");
    ...
    cleaningKernelNoOcc<<<grid, block, 0, stream>>>(texSurfaceObj, depthBuffer);
