 ...
 * @brief Kernel 4a: Direct rasterization for small entities (1 thread per entity)
 * 
 * Uses the first version approach: each thread handles one small entity completely.
...
// ============================================================================
// Small Sphere Rasterization Kernel
// ============================================================================
    ...
    cudaSurfaceObject_t outputImage)
{
    ...
    // Compute bbox (simplified for small entities)
...
// ============================================================================
// Small Cylinder Rasterization Kernel
// ============================================================================
    ...
    cudaSurfaceObject_t outputImage)
{
    ...
    glm::vec3 e = radius * glm::sqrt(glm::max(glm::vec3(0.001f), 
                   glm::vec3(1.0f) - axis * axis / axisLenSq));
    ...
    cudaStream_t stream)
{
    ...
    dim3 block(SMALL_RASTER_BLOCK_SIZE);
    dim3 grid((smallSphereCount + block.x - 1) / block.x);
    
    smallSphereRasterKernel<<<grid, block, 0, stream>>>(
        d_spheres, d_smallSphereIndices, smallSphereCount, d_depthBuffer, outputImage);
}
    ...
    cudaStream_t stream)
{
    ...
    smallCylinderRasterKernel<<<grid, block, 0, stream>>>(
        d_cylinders, d_spheres, d_smallCylinderIndices, smallCylinderCount, d_depthBuffer, outputImage);
}
