...
# Typography (bumped again for thesis readability)
FONT_SUPTITLE = 18
FONT_FACET_TITLE = 15
FONT_AXIS_LABEL = 15
FONT_TICK = 13
...
def _panel_col_wrap(n_panels: int) -> tuple[int, bool]:
...
def _apply_thesis_chart_style(
    ...
    *,
    ...
    g.set_axis_labels("Range", "Value", fontsize=FONT_AXIS_LABEL)
    ...
    g.fig.suptitle(
        f"Analysis: {rule_name} ({version_target})\nShader: {shader_target}",
...
def graficar_regla_por_columnas(
    ...
    col_wrap, legend_out = _panel_col_wrap(n_panels)
    g = sns.catplot(
        data=data_plot,
        x="Dataset",
        y="Value_Avg",
